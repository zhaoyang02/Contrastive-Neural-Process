import torch
from attrdict import AttrDict
from models.np import NP
from utils.contrastive import nt_xent_loss

class CLNP(NP):
    def __init__(self, *args, contrastive_weight=0.1, temperature=0.5, **kwargs):
        """
        Contrastive Learning Neural Process (CLNP).
        Inherits from NP and adds an auxiliary contrastive loss.
        """
        super().__init__(*args, **kwargs)
        self.contrastive_weight = contrastive_weight
        self.temperature = temperature

    def split_context(self, x, y):
        """
        Randomly splits the available context points (x, y) into two disjoint sets.
        """
        batch_size, num_points, _ = x.shape
        
        # Create a random permutation of indices
        perm = torch.randperm(num_points)
        
        # Split indices into two halves
        mid = num_points // 2
        idx1 = perm[:mid]
        idx2 = perm[mid:]
        
        # Gather data for View 1
        x1 = x[:, idx1, :]
        y1 = y[:, idx1, :]
        
        # Gather data for View 2
        x2 = x[:, idx2, :]
        y2 = y[:, idx2, :]
        
        return x1, y1, x2, y2

    def forward(self, batch, num_samples=None, reduce_ll=True):
        """
        Modified forward pass to include Contrastive Auxiliary Loss.
        """
        # 1. Standard NP Forward Pass (ELBO Calculation)
        # We call super().forward() to handle the standard ELBO logic seamlessly.
        outs = super().forward(batch, num_samples, reduce_ll)
        
        # Only compute contrastive loss during training
        if self.training:
            # 2. Contrastive Learning Step
            # Use 'batch.x' (all available points) to generate two views
            x1, y1, x2, y2 = self.split_context(batch.x, batch.y)
            
            # Ensure we have enough points to split
            if x1.shape[1] > 0 and x2.shape[1] > 0:
                # Encode View 1
                qz1 = self.lenc(x1, y1)
                z1 = qz1.loc # Use the mean of the latent distribution
                
                # Encode View 2
                qz2 = self.lenc(x2, y2)
                z2 = qz2.loc # Use the mean of the latent distribution
                
                # Compute Contrastive Loss
                cl_loss = nt_xent_loss(z1, z2, temperature=self.temperature)
                
                # Add to total loss
                outs.loss += self.contrastive_weight * cl_loss
                outs.cl_loss = cl_loss # Store for logging
                
        return outs